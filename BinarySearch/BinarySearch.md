...
## How it works?
- Set the lower bound (**low**) to the first index and the upper bound (**high**) to the last index of the sorted array, the search space is the entire array
- Calculate the middle index (**mid**) of the current search space, tipically done by the formula: **mid = (low + high) / 2**
- Compare the value at the mid index with the target value:
...
- Steps 2 and 3 are repeted until the targert is found or the search space is exhausted (which happens when the **low** index becomes greater than the **high** index)
